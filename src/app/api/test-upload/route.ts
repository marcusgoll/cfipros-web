...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    // Initialize Supabase client for auth check - await cookies()
...
async function processOcrInBackground(
  ...
  try {
    ...
    if (ocrResult.status === 'success') {
      console.log(`OCR processing successful for file ${fileId}, extracted ${ocrResult.rawText?.length || 0} characters`);
    } else {
