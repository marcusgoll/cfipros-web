...
jest.mock('@/services/test-upload-client-service', () => ({
  uploadFiles: jest.fn(),
}));
...
describe('useMultiFileUploader', () => {
  ...
  beforeEach(() => {
    ...
    jest.mocked(uploadService.uploadFiles).mockResolvedValue({
      files: [
        {
          success: true,
          originalName: 'test-uuid-1234',
  ...
  it('should handle upload failures', async () => {
    ...
    jest.mocked(uploadService.uploadFiles).mockResolvedValue({
      files: [{ success: false, originalName: 'test-uuid-1234', error: 'Server error' }],
  ...
  it('should calculate overall progress correctly', async () => {
    ...
    jest.mocked(uploadService.uploadFiles).mockImplementationOnce(async (files, onProgress) => {
      if (onProgress) {
  ...
  it('should not upload files with error status', async () => {
    ...
    const uploadMap = jest.mocked(uploadService.uploadFiles).mock.calls[0][0];
