...
describe('Gemini OCR Service', () => {
  ...
  describe('processDocumentWithOcr', () => {
    ...
    it('should handle Gemini API errors', async () => {
      ...
      const getGenerativeModelMock = jest.fn().mockReturnValue({ generateContent: generateContentMock });
      const GoogleGenerativeAIMock = jest.mocked(googleAI.GoogleGenerativeAI);
      GoogleGenerativeAIMock.mockImplementationOnce(() => ({ getGenerativeModel: getGenerativeModelMock }));
  ...
  describe('processDocumentWithRetry', () => {
    it('should return successful result without retries when successful on first attempt', async () => {
      ...
      const generateContentMock = jest.mocked(googleAI.GoogleGenerativeAI().getGenerativeModel().generateContent);
    ...
    it('should retry on API unavailability errors', async () => {
      ...
      const getGenerativeModelMock = jest.fn().mockReturnValue({ generateContent: generateContentMock });
      const GoogleGenerativeAIMock = jest.mocked(googleAI.GoogleGenerativeAI);
      GoogleGenerativeAIMock.mockImplementation(() => ({ getGenerativeModel: getGenerativeModelMock }));
