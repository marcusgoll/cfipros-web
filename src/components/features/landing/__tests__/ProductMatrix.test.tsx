...
describe('ProductMatrix', () => {
  ...
  it('renders product matrix with features', () => {
    ...
    const extractQuestionsButton = screen.getByRole('button', {
      name: /Learn more about Extract Questions/i,
    });
    ...
    const performanceButton = screen.getByRole('button', {
      name: /Learn more about Performance Analysis/i,
    });
  ...
  it('shows file upload UI in idle state', () => {
    ...
    expect(
      screen.getByText(/Upload your FAA Knowledge Test results \(PDF\) or drop it here/i)
    ).toBeInTheDocument();
