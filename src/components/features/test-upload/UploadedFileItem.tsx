...
function getFileIcon(fileType: string) {
  if (fileType.includes('image')) {
    return <Image className="h-5 w-5" title="Image file" aria-hidden="false" aria-label="Image file" />;
  }
  if (fileType.includes('pdf')) {
    return <FileText className="h-5 w-5" title="PDF file" aria-hidden="false" aria-label="PDF file" />;
  }
  return <File className="h-5 w-5" title="Document file" aria-hidden="false" aria-label="Document file" />;
}
      ...
      <CardContent className="flex items-center justify-between p-4">
        <div className="flex items-center space-x-4 overflow-hidden">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className={`text-xs ${getStatusTextClass()}`}>
              {getStatusText()}
            </p>
          </div>
        </div>
